...
viaroute2sldf_instruct <- function(routeinst, routesum, routecoords, routename = "", existrow = 0, routeid = 1, return_sf = FALSE) {
  if (class(routeinst) == "list") {
    ...
    osrmrouteinstruct <- setNames(
      data.frame(lapply(data.frame(t(sapply(routeinst, `[`))), unlist)),
      dfnames
    )
...
viaroute2sldf_instructv5 <- function(routeinst, startrouteid = 1, return_sf = FALSE) {
  if (length(routeinst$routes$legs[[1]]$steps[[1]]$geometry) == 0) {
    ...
    osrmsldf <- sp::SpatialLinesDataFrame(
      sp::SpatialLines(unlist(unlist(lapply(
        1:length(routeinst$routes$legs),
        function(i, x) {
          lapply(1:length(x$routes$legs[[i]]$steps), function(j, x, i) {
            lapply(
              ...
              x, i, j,
              ifelse(i == 1, 0, sum(unlist(lapply(1:(i - 1), function(i, x) {
                length(x$routes$legs[[i]]$steps[[1]]$geometry)
              }, x)))) +
                ifelse(j == 1, 0, sum(unlist(lapply(1:(j - 1), function(j, x, i) {
                  length(x$routes$legs[[i]]$steps[[j]]$geometry)
                }, x, i))))
            )
          }, x, i)
        },
      ...
      data.frame(dplyr::bind_rows(lapply(1:length(routeinst$routes$legs), function(i, x) {
        dplyr::bind_rows(
          lapply(1:length(x$routes$legs[[i]]$summary), function(j, x, i) {
            prevrows <- ifelse(i == 1, 0, sum(unlist(lapply(1:(i - 1), function(i, x) {
              length(x$routes$legs[[i]]$steps[[1]]$geometry)
            }, x)))) +
              ifelse(j == 1, 0, sum(unlist(lapply(1:(j - 1), function(i, x) {
                length(x[[i]]$geometry)
              }, x = x$routes$legs[[i]]$steps))))
...
nearest_osm <- function(lat, lng, number = 1,
  ...
  if (api == 4) {
    ...
    thisspdf <- sp::SpatialPointsDataFrame(
      coords = matrix(unlist(lapply(
        url, function(x) {
    ...
    thisspdf <- sp::SpatialPointsDataFrame(
      ...
      data = cbind(
        data.frame(orig_lat = lat, orig_lng = lng),
        dplyr::bind_rows(lapply(jsondata, function(x) {
          data.frame(
            distance = x$waypoints$distance,
