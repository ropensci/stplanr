...
\textbf{stplanr}, alongside other open source projects,
could provide a more transparent and democratically accountable alternative to the
current approach which is heavily reliant on proprietary and inaccessible software.
}
...
This can be used to generate heatmaps of a road network showing the most common routes such as in Figure \ref{fig:nyc-bicycle-data} (see \href{https://github.com/ropensci/stplanr/issues/194}{\#194} in the package's issue tracker for details).
...
A larger example of this desire line creation method is illustrated in \ref{fig:flights}, which also makes use of the function \texttt{buff\_geo}, is provided below based on a large dataset of flights from New York in 2013 from the \CRANpkg{nycflights13}:
...
\CRANpkg{stplanr} is deliberately focussed on geographic transport data for three main reasons, which will influence its future development:
