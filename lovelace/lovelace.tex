...
Such a class could, to build on the example data presented in previous sections, represent all the data contained in \code{flowlines}, \code{routes_fast} and \code{routes_quiet} in a single object.
...
We have demonstrated \textbf{stplnr}'s capabilities on deliberately small datasets.
However, the third criterion of transport planning software described in the introduction --- scalability --- is vital to the future of the package's uptake in industry due to the deluge of `Big Data' sources on transport patterns.
