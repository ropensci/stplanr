...
The focus of \CRANpkg{stplanr} is on geographic transport data.
We decided to narrow the package's scope in this way for three main reasons, which will influence its future development:

\begin{itemize}
\tightlist
  \item Spatial transport data have grown rapidly in recent decades, creating demand for open source software to make use of it.
  \item The success of the `unix approach' to software development, whereby each piece of software does a limited number of things, but does them well.
  \item Because the package authors have a particularly strong interest in and knowledge of spatial transport data for our own research.
\end{itemize}
