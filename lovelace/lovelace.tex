...
Another example is the potential development of a `\code{OD}' class to represent origin-destination data, which could ease the geographical analysis of OD data via generic methods.
...
These include integration with transport databases that provide fast access to large international datasets (e.g. via the packages \CRANpkg{bikedata} and \CRANpkg{osmdata}, the implementation of interfaces to batch routing APIs and the refactoring of functions which are found to be slow (e.g. \code{overline}, which takes hours to run on large datasets).
