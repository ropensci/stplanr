...
\textbf{stplanr} is a package that provides transport planning functionality
in R that meets each of these criteria.
The focus is on spatial data operations commoly required in the field,
including: tools to import and clean transport
datasets; the creation of geographic `desire lines' from
origin-destination (OD) data; route assignment of desire lines to the
transport network, e.g.~via interfaces to routing services such as
CycleStreets.net; functions to calculate the geographic attributes of such routes,
