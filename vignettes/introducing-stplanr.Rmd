...
The following command creates desire lines for all OD pairs stored --- omitting 'internal flows' via the `sel` object belwo --- represented in the dataset `flowlines`:
...
```{r, eval=curl::has_internet(), message=FALSE}
# if the next line returns FALSE this uses local data - if TRUE it pings the OSRM API
curl::has_internet() 
