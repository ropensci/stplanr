...
Specifically, we wanted to visualise and investigate the spatial distribution origin-destination (OD) data such as the open datasets provided by the UK Data Services WICID portal (see [wicid.ukdataservice.ac.uk/](http://wicid.ukdataservice.ac.uk/)).
...
The latest version can be installed from CRAN in the usual way:
...
The message printed when **stplanr** is loaded tells us that it also loads **sp**. This means that **stplanr** *depends* on on **sp**.
...
Work is needed to convert the OD data into 'desire lines'. Desire lines are straight lines between the origin and destination and represent where people would go if they were not constrained by the route network (see the Figure below, from [this paper](https://www.jtlu.org/index.php/jtlu/article/view/862)):
