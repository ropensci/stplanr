...
Origin-destination (OD) datasets contain details of trips between two geographic points.
The start and end points are known.
Usually there is a limited knowledge about the mode(s), though usually only whichever mode the agent regards as the "principle" mode.
There may be trip counts at different time periods.
If there is only one time period, then this resides in the metadata for the whole data set.
That might be train in a cycle-train-walk trip.
...
You may need to do some 'data carpentry' before the OD data is and ready for analysis.
