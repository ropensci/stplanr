...
# Function to define bands
define_band <- function(value) {
  if (value >= 0 && value <= 100) return("0-100")
  if (value > 100 && value <= 500) return("100-500")
  if (value > 500 && value <= 1000) return("500-1000")
  if (value > 1000 && value <= 5000) return("1000-5000")
  return("Other")
}
rnet_y$band <- sapply(rnet_y$value, define_band)
rnet_merged$band <- sapply(rnet_merged$value, define_band)
joined_data <- st_join(rnet_y, rnet_merged, join = st_intersects)
# Filter rows with matching bands
matching_bands <- subset(joined_data, band.x == band.y)

# Check for mismatches within each band
matching_bands$mismatch <- ifelse(matching_bands$value.x != matching_bands$value.y, 1, 0)

# Count mismatches
mismatch_count <- sum(matching_bands$mismatch)

# Display mismatch count
cat("Number of mismatches within bands:", mismatch_count, "\n")

# If you wish to see rows with mismatches
mismatch_rows <- subset(matching_bands, mismatch == 1)
print(mismatch_rows)
