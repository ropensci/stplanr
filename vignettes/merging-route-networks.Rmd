...
color_factor <- cut(rnet_y$all_fastest_bicycle, breaks=brks, labels=colors, include.lowest=TRUE)

rnet_merged = rnet_merge(rnet_x, rnet_y, dist = 20, segment_length = 10, funs = funs, max_angle_diff = 20)
# st_write(rnet_merged, "data/3km_exmaple_merged.geojson", driver = "GeoJSON")
...
library(tmaptools)
ggplot(data = rnet_y) +
  geom_sf(aes(color = all_fastest_bicycle), size = 1) +
  scale_color_gradientn(colors = colors, 
                        values = scales::rescale(c(brks[1], brks, brks[length(brks)])),
                        breaks = brks,
                        labels = brks) +
  theme_minimal() +
  labs(title = "All Fastest Bicycle Routes", 
       color = "Speed Value") +
...
summary(rnet_merged$all_fastest_bicycle_go_dutch)
summary(rnet_y$all_fastest_bicycle_go_dutch)
```
